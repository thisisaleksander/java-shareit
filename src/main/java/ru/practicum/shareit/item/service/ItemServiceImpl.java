...
public class ItemServiceImpl implements ItemService {
    ...
    @Override
    public Collection<ItemWithBooking> getItemsBy(long userId, int from, int size) throws ValidationException {
    ...
    public Comment addComment(long userId, Comment comment, long itemId) throws ValidationException {
        ...
        if (!bookings.isEmpty()) {
            comment.setItem(itemRepository.findById(itemId)
                    .orElseThrow(() -> new ItemNotFoundException(itemId)));
            comment.setAuthorName(userService.getByUserId(userId).getName());
    ...
    @Override
    public List<ItemDto> findByText(String query, int from, int size) throws ValidationException {
        if (from < 0) {
            throw new ValidationException("from parameter should be positive");
        }
        Sort sortByDate = Sort.by(Sort.Direction.ASC, "id");
        ...
        List<Item> userItems = itemRepository.getItemByNameContainingIgnoreCaseOrDescriptionContainingIgnoreCase(
                query,
                query,
                page);
